...
bucket<H>::bucket() 
{
    first_pail = (pail*) malloc(sizeof(struct pail));
}
template <class H>
bucket<H>::~bucket()
{
...
template <class H>
H bucket<H>::empty_bucket()
{
    H temp = first_pail->value;
    return temp;
}
...
//         ~llist();
//         // void clear(void);
//         // void transferFrom(llist& source);
//         // llist& operator = (llist& rhs);
//         // probably needs <= operator defined for all objects. 
...
int main()
{
    ...
    for (int i=0; i<40; i++)
    {
        // mylist.insert_item(rand() -49);
        mylist.insert_item(double(rand())/double((RAND_MAX)) * a);
    }
    mylist.print_list();
    int search_and_remove = 2.31456;
    mylist.remove(search_and_remove);
    mylist.print_list();
    std::cout << mylist.search_by_index(10) << std::endl;




    bucket<int> new_bucket;
    new_bucket.fill_bucket(12);
    int temp = new_bucket.empty_bucket();
